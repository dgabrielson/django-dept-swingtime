...
def discover_cli_scripts(path, name=None):
    ...
    pyfiles = [
        os.path.splitext(f)[0] for f in listdir
    ...
    scripts = [
        name + s for s in pyfiles if is_valid_cli_command(appname, name + s)
    ]
    listpath = [os.path.join(path, f) for f in listdir]
    sublist = [(subname, subpath)
...
def print_available_commands(app_name):
    ...
    try:
        ...
        print(
            'There was an error loading the CLI script module.',
            file=sys.stderr)
        return
...
class Command(BaseCommand):
    ...
    def run_from_argv(self, full_args):
        ...
        if option_list is not None:
            option_list = list(getattr(BaseCommand, 'option_list',
                                       [])) + list(option_list)
