...
WEEKDAY_SHORT = ((7, _(u'Sun')), (1, _(u'Mon')), (2, _(u'Tue')),
                 (3, _(u'Wed')), (4, _(u'Thu')), (5, _(u'Fri')), (6,
                                                                  _(u'Sat')))

WEEKDAY_LONG = ((7, _(u'Sunday')), (1, _(u'Monday')), (2, _(u'Tuesday')),
                (3, _(u'Wednesday')), (4, _(u'Thursday')), (5, _(u'Friday')),
                (6, _(u'Saturday')))
...
ORDINAL = ((1, _(u'first')), (2, _(u'second')), (3, _(u'third')),
           (4, _(u'fourth')), (-1, _(u'last')))
...
def timeslot_options(interval=swingtime_settings.TIMESLOT_INTERVAL,
    ...
    Note that this is for form entry -- ignore timezone issues.
    '''
    #     dt = utils.force_aware(datetime.combine(date.today(), time(0)))
    #     dtstart = datetime.combine(dt.date(), start_time)
    dt = datetime.combine(date.today(), time(0))
    dtstart = datetime.combine(dt, start_time)
...
def timeslot_offset_options(
    ...
    Note that this is for form entry -- ignore timezone issues.
    '''
    #     dt = utils.force_aware(date.today())
    #     dtstart = utils.force_aware(datetime.combine(dt.date(), start_time), dt.tzinfo)
    dt = datetime.combine(date.today(), time(0))
    dtstart = datetime.combine(dt, start_time)
    dtend = dtstart + end_delta
    options = []
...
class MultipleOccurrenceForm(forms.Form):
    day = forms.DateField(
        label=_(u'Date'), initial=date.today, widget=SelectDateWidget())
    ...
    def __init__(self, *args, **kws):
        ...
        if dtstart:
            dtstart = dtstart.replace(
                minute=(
                    (dtstart.minute // MINUTES_INTERVAL) * MINUTES_INTERVAL),
                second=0,
                microsecond=0)

            dtstart = utils.force_aware(dtstart)
            ...
            offset = (dtstart - utils.force_aware(
                dtstart.date(), timezone=dtstart.tzinfo)).seconds
            self.initial.setdefault('start_time_delta', u'%d' % offset)
            self.initial.setdefault('end_time_delta',
                                    u'%d' % (offset + SECONDS_INTERVAL, ))

            # DCG: added this to correctly set the day and until fields given by dtstart
            self.initial.setdefault('day', dtstart)
            self.initial.setdefault('until', dtstart)
    ...
    def _build_rrule_params(self):
        ...
        if self.cleaned_data['repeats'] == 'count':
            ...
            params['until'] = utils.force_aware(data['until'])
...
class SingleOccurrenceForm(forms.ModelForm):
    ...
    def validate_start_time(self):
        ...
        return super(SingleOccurrenceForm, self).validate_start_time()
...
class LocationSelectForm(forms.Form):
    """
    Note: in the template, this form needs
        id="location-select-form"

    """
    location = forms.ModelChoiceField(
        BookingLocation.objects.none(), label='Room')
    location.widget.attrs[
        "onChange"] = "document.getElementById('location-select-form').submit()"
